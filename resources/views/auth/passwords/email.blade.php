...
<div class="p-3 w-100">
    ...
    <div class="row no-gutters justify-content-center">
        <div class="col-sm-8 col-xl-6">
            <form method="POST" class="js-validation-signin" action="{{ route('password.email') }}">
                @csrf
                @if (session('status'))
                ...
                <div class="py-3">
                    <div class="form-group">
                        <input id="email" type="email" class="form-control form-control-lg form-control-alt @error('email') is-invalid @enderror" name="email" value="{{ old('name') }}" required autocomplete="email" placeholder="{{trans('auth.inputEmail')}}" autofocus>
                        @error('email')
